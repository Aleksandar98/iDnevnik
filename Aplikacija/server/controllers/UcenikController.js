...
router.post('/dodajUcenika', (req, res) => {
    ...
    const noviUcenik = new Ucenik({Ime, Prezime, Jmbg, DatumRodjenja, Adresa, Email, Razred, Odeljenje, Vladanje, Napomene, Dogadjaji, Raspored, Izostanci, Predmeti, Post,Pol});
    noviUcenik.save()
...
router.post('/update/:id', (req, res) => {
    Ucenik.findByIdAndUpdate(req.params.id, { Ime: req.body.ime}, (err, result) => {
        if (err) {
            res.send(err);
        } else {
            res.send(result);
...
router.post('/dodajIzostanak',(req,res) => {
    Ucenik.findByIdAndUpdate(req.body.id_deteta,
...
router.post('/updateIzostanak', (req, res) => {
    ...
    Ucenik.update({ _id: req.body._id ,'Izostanci._id':req.body.izostanakId }, {
