...
const Ocene1 = props => (
    <tr style={{color: "#000000"}} key={props._id}>{props.predmeti.map((predmet,index) => {
    return <td id={predmet.Naziv} style={{height:40}} key={index}>{predmet.Ocene.map(value => {
        if(value.Polugodiste==='prvo')
...
const Ocene2 = props => (
    <tr key={props._id}>{props.predmeti.map((predmet,index) => {
    return <td id={predmet.Naziv + 'r'} style={{height:40}} key={index}>{predmet.Ocene.map(value => {
        if(value.Polugodiste==='drugo')
        return value.Vrednost + ' ';
    })}</td>
    })}</tr>
)
...
export default class Statistika extends Component {
    ...
    componentDidMount() {
        axios.get('http://localhost:5000/ucenici/ucenik/'+this.props.match.params.id)
        .then(res => {
    ...
    posaljiMejlRoditelju(mejl,ocena){
        ...
        axios.post('http://localhost:5000/posaljiMejl',{
            mejlRoditelja: mejl,
            tekstPoruke: "Vase dete je dobilo zakljucnu ocenu " + ocena + ' iz predmeta ' + this.state.predmetZakljuci
        })
        .then(res => {
    ...
    zakljuci1() {
        ...
        // let hoh1 = document.getElementById(str1)
      ...
      <li><a href="/PromenaLozinke"><span className="glyphicon glyphicon-cog"></span> Promena lozinke</a></li>
            <li><a href="http://localhost:5000/logout">Odjavi se</a></li>
                    ...
                    <div className="container"> 
                        <h2 style={{color: "#000000"}}>{this.state.ucenik.Ime} {this.state.ucenik.Prezime}: {this.state.ucenik.Razred}/{this.state.ucenik.Odeljenje}, Polugodiste: 1</h2>
                        <table style={{borderColor: "#e3dd2b"}} className="table table-bordered table-striped">
                        <thead>
                            <tr>
                                {this.state.predmeti.map((predmet,ind) => {
                                    return <th style={{borderColor: "#e3dd2b", color:"#000000"}} key={ind}>{predmet.Naziv}</th>
                                })}
                            </tr>
                        </thead>
                        <tbody>
                        ...
                        <div style={{width:"33%", height:"100%"}}>
                            ...
                            <input style={{marginBottom:"2px"}} id='prvopolugodje'  type="number" class="form-control" placeholder={str} />
                            <span class="input-group-btn">
                                <button disabled={zaDugme1}   id='zak1'  style={{height:"34px", marginBottom:"10px"}} class="btn btn-success" type="button" onClick={this.zakljuci1}>Zakljuci</button>
                            </span>
                            </div>
                        </div>        

                        {
                            !nizOcena.ZakljucnaOcena[0] ? <div style={{width:"100%"}}>
                            <h3 style={{color:"purple"}}>Trenutna prosecna ocena za predmet {this.state.predmetZakljuci} iznosi: {stampaj1}</h3>
                        </div>     : <h3 style={{color:"#eb5456"}}>Ocena je zakljucena i iznosi {terzica}</h3>
                        }
                        ...
                        <h2 style={{color: "#000000"}}>{this.state.ucenik.Ime} {this.state.ucenik.Prezime}: {this.state.ucenik.Razred}/{this.state.ucenik.Odeljenje}, Polugodiste: 2</h2>
                        <table style={{borderColor: "#e3dd2b"}} className="table table-bordered table-striped">
                        <thead >
                            <tr>
                                {this.state.predmeti.map((predmet,ind) => {
                        ...
                        <div style={{width:"33%"}}>
                            <div class="input-group">
                            <input id='drugopolugodje'  type="number" class="form-control" placeholder={str} />
                            <span class="input-group-btn">
                                <button disabled={zaDugme2} style={{height:"34px"}} class="btn btn-success" type="button" onClick={this.zakljuci2}>Zakljuci</button>
                            </span>
                            </div>
                        </div> 
                        {
                            !nizOcena.ZakljucnaOcena[1] ? <div style={{width:"100%"}}>
                            <h3 style={{color:"purple"}}>Trenutna prosecna ocena za predmet {this.state.predmetZakljuci} iznosi: {stampaj2}</h3>
                        </div>     : <h3 style={{color:"#eb5456"}}>Ocena je zakljucena i iznosi {terzica1}</h3>
                        }
