...
class Prisutni extends Component{
    ...
    promeniStanje(index){
        ...
        if(noviNiz[index] == null)
        noviNiz[index] = true;
        else if(noviNiz[index] == true)
        noviNiz[index] = false;
        else
        noviNiz[index] = true;
    ...
    posaljiIzostanke(){
       ...
       this.state.indexiIzostalihUcenika.filter((vrednost)=>{ return vrednost!=undefined}).forEach((item,index)=>{
    ...
    upisiIzostanakNaCekanju(dete){
        ...
        axios.post('http://localhost:5000/ucenici/dodajIzostanak',{
            id_deteta: dete._id,
            izostanak: izostanak
        })
        .then(res => {
...
<div style={{ height: '722px'}}>
                ...
                {  this.state.ucenici.map((ucenik, index) => {
                        return (
                            <ListGroup.Item action variant={this.state.izostaliUcenici[index] ? "danger":"info"} key={index}>
                              {ucenik.Ime +" " +ucenik.Prezime}
                              <label></label>
                              <Button variant="info" style={{marginLeft: '2rem' }} onClick={() =>{ this.promeniStanje(index); }}>Nije Prisutan</Button>
                            </ListGroup.Item>
                        )
                    })
                }
                ...
                <Button variant="danger" onClick={() =>{ this.posaljiIzostanke(); }}>Upisi Izostanke</Button>
                </div>
            </div>
            ...
            // <div style={{backgroundColor: 'silver', height: '722px'}}>
            //     <div className='container'>
            //     <ListGroup variant="flush">
