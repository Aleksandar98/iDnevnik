    ...
    render() {
        ...
        dani.forEach(d => {
            if (th.length == danasnjiDan ) {
                th.push(React.createElement("th", {style: { backgroundColor: 'red' } }, d.Dan));
            }
            else
                th.push(React.createElement("th", { className:"head" }, d.Dan));
            if (granica < d.Predmeti.length)
                granica = d.Predmeti.length;
        ...
        for ( i = 0; i < granica; i++){ 
            let td = [];
            td.push(React.createElement("td", {className:"head", style:{font: "bold"}},  i+1))

            dani.forEach(dan=>{
                td.push(React.createElement("td", { className:"head" }, dan.Predmeti[i]));
            })
            tr[i] = <tr>{td}</tr>
        }
