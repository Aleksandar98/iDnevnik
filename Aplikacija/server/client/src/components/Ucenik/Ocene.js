...
class Ocene extends Component {
    ...
    drawNumbers() {
        ...
        for (let i = 1; i <= brojRazreda; i++) {
            li.push(<li key={i} className={this.state.razred ==  i  ? "active" : ""}><a onClick={() => this.changeRazred(i)} href="#">{i}</a></li>);
        }
        return li;
    }
...
//ulogovani roditelj ima id svoga deteta i onda se izvlace podaci o njegovom detetu
//i popunjava informaije o njemu
//------------------------------------------------
//kada je dete ulogovano samo prikazuje inforamcije o njemu
//IzvuciDete() {
    //let idDeteta = this.state.roditelj[0].Deca;
   // let f = "http://localhost:5000/Dete/" + idDeteta;
    
    // let f = "http://localhost:5000/Dete/5eed2675f664ad3668ee1523";
    // fetch(f)
    //     .then(res => res.json()
    ...
    render() {
        ...
        sviPredmeti.forEach((predmet, i) => {
            ...
            predmet.Ocene.forEach(ocena => {
                if (ocena.Razred == this.state.razred && ocena.Polugodiste == this.state.polugodiste)
                    odgovarajuceOcene.push(ocena.Vrednost);
            })

            predmet.ZakljucnaOcena.forEach(z => {
                if (z.Razred == this.state.razred && z.Polugodiste == this.state.polugodiste) {
            ...
            odgovarajuciPredmeti.push(React.createElement("td", null, odgovarajuceOceneString));
            odgovarajuciPredmeti.push(React.createElement("td", null, pom));
            odgovarajuciPredmeti.push(React.createElement("td", null, odgovarajucaZakljucnaOcena));
        ...
        console.log(odgovarajuceZakljucneOcene);
            ...
            proseks.forEach(p => {
                if (p.Razred == this.state.razred && p.Polugodiste == this.state.polugodiste && this.state.prosek=="/" ) {
    ...
    return (
        <div className="visina MT" >
            ...
            { odgovarajuceZakljucneOcene.length == odgovarajuciPredmetiCount ? <h3 style={{ color: "red" }}> Prosek: {this.state.prosek}</h3> : null}
        </div>  
